...
class SkillPlan (private val game: Game, private val campaign: Campaign) {
	 ...
	 *
	 * This strategy calculates optimal skills based on a community tier list and evaluates
	 * them based on their rank-to-price ratio. It filters skills to match user-specified
	...
	private fun getSkillsToBuyOptimizeSkillsStrategy(
		...
		val preferredTrackSurface: TrackSurface? = when (skillSettingTrackSurfaceString.lowercase()) {
			"no_preference" -> null
	 ...
	 *
	 * This strategy maximizes total rank by purchasing skills with the highest rank-to-price ratio.
	 * User-specified skill aptitudes are ignored in this strategy.
	 *
	 * @param skillPlanSettings The [SkillPlanSettings] to follow.
	 * @param skillList The [SkillList] to analyze.
	 * @param skillsToBuy The list of skills already planned for purchase.
	...
	private fun getSkillsToBuyOptimizeRankStrategy(
		...
		while (remainingSkills.any { it.value.screenPrice <= remainingSkillPoints }) {
			MessageLog.v(TAG, "\nChecking skills. Iteration #$i.\n")
			val sortedByPointRatio: List<SkillListEntry> = remainingSkills.values
	 ...
	 *
	 * This method orchestrates the full flow: identifying affordable skills based on
	 * the user's settings and then interacting with the game UI to buy them.
	 *
	...
	fun start(skillPlanName: String? = null): Boolean {
		...
		val skillsToPurchase: Map<String, Int> = getSkillsToBuy(
			skillPlanSettings = skillPlanSettings,
			skillList = skillList,
